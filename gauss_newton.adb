--  Gauss_Newton body — undamped nonlinear least squares with optional
--  simple backtracking line search (Wikipedia Gauss–Newton algorithm).

pragma Ada_2022;

with Ada.Numerics.Generic_Elementary_Functions;

package body Gauss_Newton
  with SPARK_Mode => Off
is
   ...
   begin
      ...
      for Iter in 1 .. Cfg.Max_Iterations loop
         ...
         --  Undamped Gauss–Newton step: (JᵀJ) δ = −Jᵀ r
         Solved := True;
         begin
            Step_Vec := Solve_SPD (A, Scale (-1.0, Grad));
         exception
         ...
         Improved := False;
         for Attempt in 0 .. Max_BT - 1 loop
            if Cfg.Use_Line_Search then
               Alpha := Backtrack_Alpha (1.0, Cfg.Line_Search_Rho, Attempt);
            else
               Alpha := 1.0;
            end if;

            Trial := Add (Beta, Scale (Real (Alpha), Step_Vec));
            declare
            ...
            if New_Cost < Cur_Cost
              or else (Attempt = 0 and then Near (Real (New_Cost),
                                                  Real (Cur_Cost),
                                                  Real (Cfg.Cost_Tol)))
            then
               Improved := True;
               Accepted_Once := True;
               Step_Norm := Norm2 (Scale (Real (Alpha), Step_Vec));
               Rel_Imp :=
            ...
            --  Without line search, do not retry.
            exit when not Cfg.Use_Line_Search;
         end loop;
